# 配置说明

## 加载项配置项

| 字段 | 示例 | 说明 |
| --- | --- | --- |
| `local_ip` | `192.168.16.64` | HA 主机在门禁网络中的 IP。必须真实存在于 HA 网卡上。 |
| `local_id` | `00010116010` | 虚拟室内机 ID。当前 1601 使用此值。 |
| `building_id` | `1栋A座` | 楼栋预设。会按楼栋规则表生成门口机 IP。 |
| `center_ip` | `192.168.16.2` | 小区中心地址。 |
| `property_center_ip` | `192.168.16.3` | 物业中心机地址。 |
| `api_host` | `0.0.0.0` | 后端接口监听地址。 |
| `api_port` | `8099` | 后端接口监听端口。 |
| `api_token` | 空 | 动作接口访问令牌。为空时解锁/接听接口不可用。 |
| `door_01_ip` - `door_08_ip` | 空 | 可选门口机 IP 覆盖。留空时使用当前楼栋预设；填写后覆盖对应号机。 |

## 楼栋 ID

配置页下拉框会直接显示中文楼栋名。程序仍兼容旧版本保存过的内部值，例如 `building_1_a`。

当前内置楼栋：

```text
building_1_a -> 1栋A座
building_1_b -> 1栋B座
building_1_c -> 1栋C座
building_1_d -> 1栋D座
building_1_e -> 1栋E座
building_2_a -> 2栋A座
building_2_b -> 2栋B座
building_2_c -> 2栋C座
```

当前各楼栋都已按你提供的规则表生成，未记录的号机不会出现在配置里，也不会参与监听。

## 门口机 IP 覆盖

加载项配置页里有 `door_01_ip` 到 `door_08_ip`。这些字段默认留空，表示使用 `building_id` 对应的内置楼栋 IP 规则。

如果现场抓包发现某个号机 IP 和预设不一致，只需要填写对应字段。例如只覆盖 2 号机：

```yaml
door_02_ip: "192.168.16.250"
```

没有出现在当前楼栋规则里的号机不会因为填写覆盖项而被创建。例如 2栋C座的 5号机目前缺失，`door_05_ip` 留空即可；后续确认后再填。

## 1 栋 A 座门口机

| 号机 | IP | 楼层 | 位置 |
| --- | --- | --- | --- |
| 1号机 | `192.168.16.224` | 1层 | 车库 |
| 2号机 | `192.168.16.225` | 2层 | 花园 |
| 3号机 | `192.168.16.226` | -1层 | 车库 |
| 4号机 | `192.168.16.227` | -2层 | 车库 |
| 5号机 | `192.168.16.228` | -1层 | 电梯左侧小门左边 |
| 6号机 | `192.168.16.229` | -1层 | 电梯左侧小门右边 |
| 7号机 | `192.168.23.164` | -2层 | 电梯左侧小门左边 |
| 8号机 | `192.168.23.165` | 1层 | 电梯正对 |

## 持久化配置文件

加载项会在数据目录中写入运行时配置文件。这个文件属于加载项数据目录，会随 HA 备份保存。后续 HA 集成可以通过服务或接口修改门口机 IP，再由核心后端读取。

当前同步规则：

- `local_ip`、`local_id`、`building_id`、`center_ip`、`property_center_ip`、接口配置以加载项配置页为准。
- `devices` 从楼栋规则表和 `door_01_ip` 到 `door_08_ip` 覆盖项生成。
- 运行时配置文件只作为当前配置镜像保存，重启后会重新同步加载项配置页。
